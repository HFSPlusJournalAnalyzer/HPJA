...
def AllocationContentParser(transaction,specialFileOffset,tranLen):
    ...
    for k in range(0,tranLen):
        for l in range(0,8):
            if start==-1 and (BytesToValue(transaction[i][j][1][k])>>(7-l))%2==1:
                ...
                allocatedBlock[0]+=1
                allocatedBlock.append((start+specialFileOffset*8,k*8+l-start))
                start=-1
    
    for k in range(1,existentFork[0]+1):

        if existentFork[k][0][0]==1:

            check=0

            if specialFileOffset*8<=existentFork[k][0][1] and existentFork[k][0][1]+existentFork[k][0][2]<=(specialFileOffset+tranLen)*8:
            ...
            elif specialFileOffset*8<=existentFork[k][0][1]<(specialFileOffset+tranLen)*8:
            ...
            elif specialFileOffset*8<existentFork[k][0][1]+existentFork[k][0][2]<=(specialFileOffset+tranLen)*8:

                check=1
                for l in range(1,allocatedBlock[0]+1):
                    if allocatedBlock[l][0]==specialFileOffset*8 and allocatedBlock[l][0]+allocatedBlock[l][1]>=existentFork[k][0][1]+existentFork[k][0][2]:
                        check=2
                        break
            ...
            for k in range(0,3):
                ...
                if check:
                    if k<3:
                        ...
                        transaction[i][j][2].append(offset)
                        ContentParser[k](transaction[i][j][1],offset,tranLen)
