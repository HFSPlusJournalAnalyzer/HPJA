...
def getForkData(fd_binary):
    ...
    for i in range(8):
        ...
        ext.append(e)
    vec = list(unpack('>QII',fd_0))
    vec.append(ExtentsDataRec(*ext))
    return ForkData(*vec)

def getOpaqueInfo(oi_binary):
    vec = unpack_from(">"+"b"*16, oi_binary)
    return OpaqueInfo(vec)
...
def getFolderInfo(foi_binary):
    ...
    vec = [Rect(*RectVec), Flag, Point(*Location), opaque]
    return FolderInfo(*vec)

def getExtendedFolderInfo(efi_binary):
    vec = unpack_from(">IIHhi", efi_binary)
    return ExtendedFolderInfo(*vec)
...
def getFileInfo(fii_binary):
    ...
    vec.extend([Point(*Location), opaque])
    return FileInfo(*vec)

def getExtendedFileInfo(efi_binary):
    vec = unpack_from(">IIHhi", efi_binary)
    return ExtendedFileInfo(*vec)
...
def getCatalogThread(cth_binary):
    ...
    return CatalogThread(recordType, reserved, parID, UniChar(nameLen, nodeUnicode))
...
def getCatalogKey(ck_binary):
    ...
    return CatalogKey(rType, parID, UniChar(nameLen, nodeUnicode))
...
def getCatalogLeafRec(clk_binary):
    ...
    return BTRecord(catalKey, Record)
...
def getCatalogPointerRec(cpr_binary):
    ...
    return BTRecord(catalKey, BTPointer(nodeNum))
...
def getExtentsLeafRec(elf_binary):
    ...
    return BTRecord(extKey, ExtentsDataRec(*ext))
...
def getExtentsPointerRec(epr_binary):
    ...
    return BTRecord(extKey, BTPointer(nodeNum))
...
def getAttributesForkData(af_binary):
    ...
    return AttrForkData(recordType, res, fd)
...
def getAttributesExtents(ae_binary):
    ...
    for i in range(8):
        ...
        e = getExtentDescriptor(temp)
        ext.append(e)
    return AttrExtents(recordType, res, ExtentsDataRec(*ext))
...
def getAttributesPointerRec(apr_binary):
    ...
    return BTRecord(attKey, BTPointer(nodeNum))
...
def getHeaderNode(ch_binary): 
    ...
    return HeaderNode(nd, hr, udr, mr)
